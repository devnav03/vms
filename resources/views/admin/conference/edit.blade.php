                        ...
                        <div class="row col s12">
                          {!! Form::open(['route'=>['admin.'.request()->segment(2).'.update', $menus->id], 'method'=>'put','id'=>'menuForm','class'=>'col s12']) !!}
                            <div class="form-group col s6">
                                ...
                                <div class="col-md-9">
                                    {!! Form::text('room',$menus->room, ['class'=>'form-control','required']) !!}
                            ...
                            <div class="col-md-9">
                                  ...
                                  <option value="{{$location->id}}" {{ ($location->name == $menus->getLocation->name) ?'selected':''}}>{{$location->name}}</option>
                                  @endforeach
                              </select>
                              ...
                              @if($errors->has('location_id'))<p class="text-danger">{{ $errors->first('location_id') }}</p> @endif
							            ...
							            <div class="form-group col s6" style="height: 84px;">
                                  <div class="col-md-3">
                                      {!! Form::label('name', 'Building', ['class'=>'control-label']) !!}
                                  </div>
                                  <div class="col-md-9" id="building">
                                    <input type="text" class='form-control' value="{{ $menus->getBuilding->name }}">
                                    <input type="hidden" name="building_id" value="{{ $menus->getBuilding->id }}">
                                      @if($errors->has('building_id'))<p class="text-danger">{{ $errors->first('building_id') }}</p> @endif
                                  </div>
                            </div>  
							
							              <div class="form-group col s6" style="margin-bottom: 20px;">
                                <div class="col-md-3">
                                    {!! Form::label('name', 'Department', ['class'=>'control-label']) !!}
								                </div>
                                <div class="col-md-9" id="department">
                                  <input type="text" class='form-control' value="{{ $menus->getDepartment->name }}">
                                  <input type="hidden" name="department_id" value="{{ $menus->getDepartment->id }}">
                            ...
                            <div class="form-group col s6">
                                ...
                                <input type="text" class='form-control' value="{{ $menus->getDevice->name }}">
                                    <input type="hidden" name="device_id" value="{{ $menus->getDevice->id }}">									
